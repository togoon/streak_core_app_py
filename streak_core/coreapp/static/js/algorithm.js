...
var pref_data = null;
var lastest_item = [];
var default_preferences = {'drag_container1':['at_price',						'price','number'],
							'drag_container2_ti':['sma','ema','rsi','macd','macd_signal','obv','atr','aroon_up','aroon_down'],
							'drag_container2_ohlcv':['open','high','low','close','volume'],
							'drag_container3':['higher_than','lower_than','higher_than_equal_to','lower_than_equal_to']
							};
...
function custom_click(ev){
	...
	if(bool_ti){
		ti_popup_show3(js_parsing_tree.main.indicator[data.id],ev,data.id,searched);
	}
	else{
		condition_div = $('.display_condition');
		right_div = null;
