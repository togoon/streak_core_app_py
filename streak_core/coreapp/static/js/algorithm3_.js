...
var default_preferences = {'drag_container1':['at_price','number'],
              'drag_container2_ti':['sma','ema','wma','tma','dema','tema','rsi','macd','macd_signal','obv','atr','aroon_up','aroon_down','open','high','low','close','volume','at_price','number','cci','mfi','adx','willr','parabolic_sar','plus_di','minus_di','ubb','mbb','lbb','supertrend','prev_open','prev_close','prev_high','prev_low','atr_bands_top','atr_bands_bottom'],
    ...
    if(!$(obj).val())
      ...
      select:function(event, ui){
        ...
        // if(condition_div.length==parseInt(exit_cursor_loc/3))
          ...
          latest_indicator = exit_lastest_item[0]
          // if(latest_indicator['allowed_comparision'].includes(item['function_group']) || (exit_cursor_loc/3>=1 && exit_cursor_loc%3==0 && exit_cursor_loc%3!=0))
          // {
            ev = {};
            ev['currentTarget']={'className':'ti ti_tags','classList':['ti','ti_tags'],'id':val};
            custom_click_exit(ev,event);
      ...
      $(obj).autocomplete({
        ...
        select:function(event, ui){
          ...
          // if(condition_div.length==parseInt(exit_cursor_loc/3))
            ...
            latest_indicator = exit_lastest_item[0]
            // if(latest_indicator['allowed_comparision'].includes(item['function_group']) || (exit_cursor_loc/3>=1 && exit_cursor_loc%3==0))
            // {
              ev = {};
              ev['currentTarget']={'className':'co co_tags','classList':['co','co_tags'],'id':val};
              // custom_click(ev,event);
              ...
              $.get('/autocomplete_indicators2/', params,function(data) {
                ...
                if(data['status']=='success'){
                  ...
                  response($.map(data['results'], function (el) {
                    // console.log(el)
                    item = js_parsing_tree.main.indicator[el[0].toLowerCase()];
                    if((latest_indicator['allowed_comparision'].includes(item['function_group']) || (exit_cursor_loc/3>=1 && exit_cursor_loc%3==0))&& !exit_third_cleared_flag){
                      return {
                        label: el[0].toUpperCase()+'<span>('+el[1]+')</span>',
                        value: el[0].toLowerCase().replace(/_/g,' ')//assumption, symbols and segment name does not have space in between
                      };
...
function custom_click_exit(ev,event){
  ...
  if(bool_ti){
    ti_popup_show4_exit(js_parsing_tree.main.indicator[data.id],ev,data.id,searched,search_target);
  }
  else{
    condition_div = $('.display_condition_exit');
    right_div = null;
...
function ti_popup_show4_exit(data,ev,id,searched,search_target){
    $(".indicator_popup").remove();
...
function clear_condition_first_exit(e){
  ...
  if($('.display_condition_exit').length>1){
    ...
    $(x).html('<div class="input_divs"><input type="text" name="search_ti1_exit" class="search_ti1_exit" placeholder="Enter indicator"></div><div class="input_divs"><input type="text" name="search_co_exit" class="search_co_exit" placeholder="Enter comparator"></div><div class="input_divs"><span class="third_ip_close" onclick="clear_third_indicator_exit(event)" style="display: none;"><img src="/static/imgs/icon-force-stop.png"></span><input type="text" name="search_ti2_exit" class="search_ti2_exit" placeholder="Enter indicator"></div><span class="clear" onclick="clear_condition_first_exit(event)">Clear</span>');
...
function insert_condition3_exit(event){
  ...
  {
    if($('.display_condition_exit').length<5){
      $(event.target).hide();
      $(".conditions_exit").append('<div class="display_condition_exit" id="display_condition1"><div class="and_or" onchange="and_or_select_change(event)"><select id="andor"><option>and</option><option>or</option></select></div><div class="input_divs"><input type="text" name="search_ti1_exit" class="search_ti1_exit" placeholder="Enter indicator1"></div> <div class="input_divs"><input type="text" name="search_co_exit" class="search_co_exit" placeholder="Enter comparator"></div> <div class="input_divs"><span class="third_ip_close" onclick="clear_third_indicator_exit(event)" style="display: none;"><img src="/static/imgs/icon-force-stop.png"></span><input type="text" name="search_ti2_exit" class="search_ti2_exit" placeholder="Enter indicator2"></div> <span class="clear" onclick="clear_condition_exit(event)">Remove</span></div><div class="another_condition_exit"><p onclick="insert_condition3_exit(event);">+ Add another condition</p></div>');
      show_prompter_text_exit();
      update_autocomplete_all_exit();
    }
    else{
